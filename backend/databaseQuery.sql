...
create table Case_Hearing(H_id IDENTITY,Case_id int,C_id int,Crt_id int,Stage varchar(50),Brief varchar(500),Previous_Date date,Next_Date date,Appointment_time time,L_id int,CONSTRAINT pk_Case_Hearing_H_id Primary key(H_id),CONSTRAINT fk_Case_Hearing_Profile_L_id FOREIGN KEY (L_id) references Profile(L_id),CONSTRAINT fk_Case_Hearing_Client_Profile_C_id FOREIGN KEY (C_id) references Client_Profile(C_id),CONSTRAINT fk_Case_Hearing_Court_Crt_id FOREIGN KEY (Crt_id) references Court(Crt_id),CONSTRAINT fk_Case_Hearing_Crt_Case_Case_id FOREIGN KEY (Case_id) references Crt_Case(Case_id));
